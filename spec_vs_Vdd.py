...
Vc = 1/n**alpha #ext pot amplitude for the power law potential
...
def spec_Vc( q,L,N,g,U,U2,V0,alpha,mu,J, Green_op_q, wvals, Vcvals,T, k =None,sps =None, M= 100, Gamma = 0.05, Ext_pot_q=0):
    ...
    table = np.zeros( (len(Vcvals), len(wvals)), dtype = float)
    for i,Vc in enumerate(Vcvals):
        print('Vc index is',i)
...
gvals = np.linspace(-0.5,0.5,40)
Uvals = np.linspace(0,5,40)
Vcvals = [i*0.1/n**alpha for i in range(50)]
Vcvals = [i*0.5/n**alpha for i in range(25)]
#Vcvals = [0/n**alpha for i in range(4)]
time_s = time.time()
spec_vs_Vc = spec_Vc( 0.0,L,N,g,U,U2,V0,alpha,mu,J, Green_op_q, w, Vcvals,T, k =None,sps =sps, M= M, Gamma = 0.05, Ext_pot_q=Ext_pot_q)
#spec_vs_Vc = spec_g( 0,L,N,g,U2,Vc,alpha,mu,J, Green_op_q, w, Uvals,T, k =None,sps =sps, M= 100, Gamma = 0.05, Ext_pot_q=0)
print("this took", time_s - time.time(), "secondes...")
print('Vc is ', Vc)
plot_dyn_structure_factor(L,N,U,U2,Vc,V0,alpha,g,M,spec_vs_Vc, Vcvals, w, Gamma, yaxis = r"$A(q,\Omega)$", obs = "Polaron Spectral Function", bound = 1,Ext_pot_q=Ext_pot_q,sps =sps, xaxis = 'Dipolar Vc' ) 
